...
module Admin
  class SoftwareUpdatesController < BaseController
    ...
    def index
      ...
      @upstream_update_batches = UpstreamUpdateBatch.current_source.recent_first.limit(20)
      @upstream_update_check = UpstreamUpdateCheck.find_by(repository: upstream_repository, channel: upstream_channel)
    end

    def review_upstream
      authorize :software_update, :index?
      UpstreamUpdateBatch.current_source.pending_review.find(params.require(:batch_id)).update!(reviewed_at: Time.current)

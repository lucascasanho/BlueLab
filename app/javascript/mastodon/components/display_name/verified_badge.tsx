...
const VerificationIcon: FC<{ className?: string }> = ({ className }) => {
...
const CalendarIcon: FC<{ className?: string }> = ({ className }) => (
  <svg
    ...
    focusable='false'
  >
    {/* Calendar SVG drawn for the BlueLab verification popover. */}
    <path
      d='M7 2.75v3M17 2.75v3M4.5 8.5h15M5.25 4.25h13.5A2.25 2.25 0 0 1 21 6.5v12.25A2.25 2.25 0 0 1 18.75 21H5.25A2.25 2.25 0 0 1 3 18.75V6.5a2.25 2.25 0 0 1 2.25-2.25Z'
      fill='none'
      stroke='currentColor'
      strokeWidth='1.75'
      strokeLinecap='round'
...
export function hasVerifiedRole(account: DisplayNameProps['account']) {
  ...
  return Array.from(account.roles).some((role) =>
    VERIFIED_ROLE_NAMES.has(role.name.trim().toLocaleLowerCase()),
  );
}
...
export const VerifiedBadge: FC<Pick<DisplayNameProps, 'account'>> = ({
  ...
  return (
    <>
      ...
      >
        <VerificationIcon className={classes.badge} />
      </span>
        ...
        {({ props }) => (
          ...
          >
            ...
            <div className={classes.detailRow}>
              <VerificationIcon className={classes.detailVerificationIcon} />
              <p className={classes.detailText}>{descriptionText}</p>
            </div>

            <div className={classes.detailRow}>
              <CalendarIcon className={classes.detailIcon} />
              <p className={classes.detailText}>{sinceText}</p>
            </div>
          </div>
        )}
      </Popover>

...
const getCompactLabel = (locale, hidden, fallback) => {
...
export const ThreadVisibilityToggle = ({ hidden, onClick }) => {
  const intl = useIntl();
  const message = hidden ? messages.showAll : messages.showLess;
  const label = getCompactLabel(intl.locale, hidden, intl.formatMessage(message));
...
ThreadVisibilityToggle.propTypes = {

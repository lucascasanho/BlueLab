...
class UpdateStatusService < BaseService
  ...
  def validate_media!
    ...
    media_attachments = @status.account.media_attachments.where(status_id: [nil, @status.id]).where(scheduled_status_id: nil, announcement_id: nil).where(id: @options[:media_ids].take(Status::MEDIA_ATTACHMENTS_LIMIT).map(&:to_i)).to_a
  ...
  def update_immediate_attributes!
    ...
    @status.content_type = @options[:content_type] if @options.key?(:content_type)

...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    n.item :web, safe_join([material_symbol('chevron_left'), t('settings.back', site_title: site_title)]), root_path
    ...
    n.item :moderation, safe_join([material_symbol('gavel'), t('moderation.title')]), nil, if: -> { current_user.can?(:manage_reports, :view_audit_log, :manage_users, :manage_invites, :manage_taxonomies, :manage_federation, :manage_blocks) && !self_destruct } do |s|
      ...
      s.item :bug_reports, safe_join([material_symbol('bug_report'), t('admin.bug_reports.title')]), admin_bug_reports_path, highlights_on: %r{/admin/bug_reports}, if: -> { current_user.can?(:manage_reports) }
    ...
    n.item :admin, safe_join([material_symbol('manufacturing'), t('admin.title')]), nil, if: -> { current_user.can?(:view_dashboard, :manage_settings, :manage_rules, :manage_announcements, :manage_custom_emojis, :manage_webhooks, :manage_federation) && !self_destruct } do |s|
      ...
      s.item :settings, safe_join([material_symbol('tune'), t('admin.settings.title')]), admin_settings_path, if: -> { current_user.can?(:manage_settings) }, highlights_on: %r{/admin/settings(?!/instance_customization)}

...
RSpec.describe 'Admin instance customization settings' do
  ...
  let(:runtime_palette) do
    {
      instance_accent_color: '#d52a96',
      instance_light_background_color: '#f6f8fa',
      instance_light_surface_color: '#e6ebf0',
      instance_light_text_color: '#20242a',
      instance_dark_background_color: '#101418',
      instance_dark_surface_color: '#15191d',
      instance_dark_text_color: '#f2f5f7',
    }
  end
  ...
  it 'emits every editable BlueLab runtime palette token' do
    sign_in Fabricate(:admin_user)
    Setting.theme = 'blue-2'
    runtime_palette.each { |setting, value| Setting.public_send("#{setting}=", value) }
    get admin_settings_instance_customization_path
  ...
  it 'emits browser-valid unescaped BlueLab attribute selectors' do
    sign_in Fabricate(:admin_user)
    Setting.theme = 'blue-2'
    runtime_palette.each { |setting, value| Setting.public_send("#{setting}=", value) }
    get admin_settings_instance_customization_path
    ...
    expect(response.body).to include('html[data-color-scheme=auto] body[data-theme=blue-2]')
